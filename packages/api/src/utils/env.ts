import { extractEnvVariable } from 'aladin-data-provider';
...
function processUserPlaceholders(value: string, user?: IUser): string {
  ...
  for (const field of ALLOWED_USER_FIELDS) {
    const placeholder = `{{ALADIN_USER_${field.toUpperCase()}}}`;
 ...
 * Recognized placeholders: `{{ALADIN_BODY_<FIELD>}}` where `<FIELD>` ∈ ALLOWED_BODY_FIELDS.
...
function processBodyPlaceholders(value: string, body: RequestBody): string {
  ...
  for (const field of ALLOWED_BODY_FIELDS) {
    const placeholder = `{{ALADIN_BODY_${field.toUpperCase()}}}`;
