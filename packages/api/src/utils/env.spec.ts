import { TokenExchangeMethodEnum } from 'aladin-data-provider';
import { resolveHeaders, resolveNestedObject, processMCPEnv } from './env';
import type { MCPOptions } from 'aladin-data-provider';
import type { IUser } from '@aladin/data-schemas';
...
describe('resolveHeaders', () => {
  ...
  it('should process full user object placeholders', () => {
    ...
    const headers = {
      'User-Email': '{{ALADIN_USER_EMAIL}}',
      'User-Name': '{{ALADIN_USER_NAME}}',
      'User-Username': '{{ALADIN_USER_USERNAME}}',
      'User-Role': '{{ALADIN_USER_ROLE}}',
      'User-Id': '{{ALADIN_USER_ID}}',
      'Content-Type': 'application/json',
    };
  ...
  it('should handle missing user fields gracefully', () => {
    ...
    const headers = {
      'User-Email': '{{ALADIN_USER_EMAIL}}',
      'User-Username': '{{ALADIN_USER_USERNAME}}',
      'Non-Existent': '{{ALADIN_USER_NONEXISTENT}}',
    };
  ...
  it('should prioritize custom user variables over user fields', () => {
    ...
    const customUserVars = {
      ALADIN_USER_EMAIL: 'custom-email@example.com',
    };

    const headers = {
      'Test-Email': '{{ALADIN_USER_EMAIL}}',
    };
  ...
  it('should handle multiple occurrences of the same placeholder', () => {
    ...
    const headers = {
      'Primary-Email': '{{ALADIN_USER_EMAIL}}',
      'Secondary-Email': '{{ALADIN_USER_EMAIL}}',
      'Backup-Email': '{{ALADIN_USER_EMAIL}}',
    };
  ...
  it('should handle mixed variable types in the same headers object', () => {
    ...
    const headers = {
      ...
      'X-User-Id': '{{ALADIN_USER_ID}}',
      'X-System-Key': '${TEST_API_KEY}',
      'X-User-Email': '{{ALADIN_USER_EMAIL}}',
      'Content-Type': 'application/json',
    };
  ...
  it('should replace all allowed user field placeholders', () => {
    ...
    const headers = {
      'X-User-ID': '{{ALADIN_USER_ID}}',
      'X-User-Name': '{{ALADIN_USER_NAME}}',
      'X-User-Username': '{{ALADIN_USER_USERNAME}}',
      'X-User-Email': '{{ALADIN_USER_EMAIL}}',
      'X-User-Provider': '{{ALADIN_USER_PROVIDER}}',
      'X-User-Role': '{{ALADIN_USER_ROLE}}',
      'X-User-GoogleId': '{{ALADIN_USER_GOOGLEID}}',
      'X-User-FacebookId': '{{ALADIN_USER_FACEBOOKID}}',
      'X-User-OpenIdId': '{{ALADIN_USER_OPENIDID}}',
      'X-User-SamlId': '{{ALADIN_USER_SAMLID}}',
      'X-User-LdapId': '{{ALADIN_USER_LDAPID}}',
      'X-User-GithubId': '{{ALADIN_USER_GITHUBID}}',
      'X-User-DiscordId': '{{ALADIN_USER_DISCORDID}}',
      'X-User-AppleId': '{{ALADIN_USER_APPLEID}}',
      'X-User-EmailVerified': '{{ALADIN_USER_EMAILVERIFIED}}',
      'X-User-TwoFactorEnabled': '{{ALADIN_USER_TWOFACTORENABLED}}',
      'X-User-TermsAccepted': '{{ALADIN_USER_TERMSACCEPTED}}',
    };
  ...
  it('should handle multiple placeholders in one value', () => {
    ...
    const headers = {
      'X-Multi': 'User: {{ALADIN_USER_ID}}, Env: ${TEST_API_KEY}, Custom: {{MY_CUSTOM}}',
    };
  ...
  it('should handle a mix of all types', () => {
    ...
    const headers = {
      'X-User': '{{ALADIN_USER_ID}}',
      'X-Env': '${TEST_API_KEY}',
      'X-Custom': '{{MY_CUSTOM}}',
      'X-Multi': 'ID: {{ALADIN_USER_ID}}, ENV: ${TEST_API_KEY}, CUSTOM: {{MY_CUSTOM}}',
      'X-Unknown': '{{NOT_A_REAL_PLACEHOLDER}}',
      'X-Empty': '',
      'X-Boolean': '{{ALADIN_USER_EMAILVERIFIED}}',
    };
    const customVars = { MY_CUSTOM: 'custom-value' };
    const result = resolveHeaders({ headers, user, customUserVars: customVars });
  ...
  it('should process ALADIN_BODY placeholders', () => {
    const body = {
      conversationId: 'conv-123',
      parentMessageId: 'parent-456',
      messageId: 'msg-789',
    };
    const headers = { 'X-Conversation': '{{ALADIN_BODY_CONVERSATIONID}}' };
  ...
  describe('non-string header values (type guard tests)', () => {
    ...
    it('should handle numeric values with placeholders', () => {
      ...
      const headers = {
        ...
        'X-String-With-Placeholder': '{{ALADIN_USER_ID}}',
      };
      const result = resolveHeaders({ headers, user });
    ...
    it('should handle numeric values with body placeholders', () => {
      ...
      const headers = {
        ...
        'X-Conv': '{{ALADIN_BODY_CONVERSATIONID}}',
      };
      const result = resolveHeaders({ headers, body });
    ...
    it('should not crash when calling includes on non-string body field values', () => {
      ...
      const headers = {
        'X-Conv-Id': '{{ALADIN_BODY_CONVERSATIONID}}',
        'X-Number': 999 as unknown as string,
      };
...
describe('resolveNestedObject', () => {
  ...
  it('should process strings in arrays', () => {
    const user = { id: 'user-123' };
    const obj = {
      headers: ['Authorization: Bearer ${TEST_API_KEY}', 'X-User-Id: {{ALADIN_USER_ID}}'],
      values: [1, 2, 3],
      mixed: ['string', 42, true, '{{ALADIN_USER_ID}}'],
    };
  ...
  it('should handle body placeholders in nested objects', () => {
    ...
    const obj = {
      metadata: {
        conversation: '{{ALADIN_BODY_CONVERSATIONID}}',
        parent: '{{ALADIN_BODY_PARENTMESSAGEID}}',
        count: 5,
      },
    };
  ...
  it('should handle arrays of objects', () => {
    ...
    const obj = {
      items: [
        { name: 'item1', user: '{{ALADIN_USER_ID}}', count: 1 },
        { name: 'item2', user: '{{ALADIN_USER_ID}}', count: 2 },
      ],
    };
  ...
  it('should not modify the original object', () => {
    ...
    expect(originalObj.thinking.user_id).toBe('{{ALADIN_USER_ID}}');
  });
});
...
describe('processMCPEnv', () => {
  ...
  it('should process user field placeholders in all fields', () => {
    ...
    const options: MCPOptions = {
      ...
      env: {
        USER_ID: '{{ALADIN_USER_ID}}',
        USER_EMAIL: '{{ALADIN_USER_EMAIL}}',
        USER_ROLE: '{{ALADIN_USER_ROLE}}',
      },
      args: ['--user', '{{ALADIN_USER_USERNAME}}', '--id', '{{ALADIN_USER_ID}}'],
    };
  ...
  it('should process body placeholders in all fields', () => {
    ...
    const options: MCPOptions = {
      type: 'streamable-http',
      url: 'https://api.example.com/conversations/{{ALADIN_BODY_CONVERSATIONID}}',
      headers: {
        'X-Parent-Message': '{{ALADIN_BODY_PARENTMESSAGEID}}',
        'X-Message-Id': '{{ALADIN_BODY_MESSAGEID}}',
      },
    };
  ...
  it('should handle mixed placeholders in OAuth configuration', () => {
    ...
    const options: MCPOptions = {
      ...
      oauth: {
        ...
        client_id: '${OAUTH_CLIENT_ID}',
        client_secret: '${OAUTH_CLIENT_SECRET}',
        scope: 'user:{{ALADIN_USER_ID}} conversation:{{ALADIN_BODY_CONVERSATIONID}}',
        redirect_uri: 'http://localhost:3000/user/{{ALADIN_USER_EMAIL}}/callback',
      },
    };
  ...
  it('should handle all placeholder types in a single value', () => {
    ...
    const options: MCPOptions = {
      ...
      env: {
        COMPLEX_VALUE:
          'User: {{ALADIN_USER_ID}}, Custom: {{CUSTOM_VAR}}, Body: {{ALADIN_BODY_CONVERSATIONID}}, Env: ${TEST_API_KEY}',
      },
    };
  ...
  describe('non-string values (type guard tests)', () => {
    ...
    it('should not crash with numeric body field values', () => {
      ...
      const options: MCPOptions = {
        ...
        args: [],
        env: {
          CONV_ID: '{{ALADIN_BODY_CONVERSATIONID}}',
          PORT: 8080 as unknown as string,
        },
      };
