...
describe('Environment Variable Extraction (MCP)', () => {
  ...
  describe('processMCPEnv', () => {
    ...
    it('should ensure different users with same starting config get separate values', () => {
      ...
      expect(baseConfig.headers?.['User-Id']).toBe('{{ALADIN_USER_ID}}');
    ...
    it('should process dynamic user fields in headers', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          ...
          'User-Name': '{{ALADIN_USER_USERNAME}}',
          OpenID: '{{ALADIN_USER_OPENIDID}}',
          'Google-ID': '{{ALADIN_USER_GOOGLEID}}',
          'Email-Verified': '{{ALADIN_USER_EMAILVERIFIED}}',
    ...
    it('should process user fields in URL', () => {
      ...
      const options: MCPOptions = {
        type: 'sse',
        url: 'https://example.com/api/{{ALADIN_USER_USERNAME}}/stream',
      };
    ...
    it('should handle boolean user fields', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          'Email-Verified': '{{ALADIN_USER_EMAILVERIFIED}}',
          'Two-Factor': '{{ALADIN_USER_TWOFACTORENABLED}}',
          'Terms-Accepted': '{{ALADIN_USER_TERMSACCEPTED}}',
        },
      };
    ...
    it('should not process sensitive fields like password', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          'User-Email': '{{ALADIN_USER_EMAIL}}',
          'User-Password': '{{ALADIN_USER_PASSWORD}}', // This should not be processed
        },
      };
      ...
      expect('headers' in result && result.headers).toEqual({
        'User-Email': 'test@example.com',
        'User-Password': '{{ALADIN_USER_PASSWORD}}', // Unchanged
      });
    });
    ...
    it('should support both id and _id properties for ALADIN_USER_ID', () => {
      ...
      expect('headers' in result2 && result2.headers?.['User-Id']).toBe('{{ALADIN_USER_ID}}');
    ...
    it('should process customUserVars in URL field', () => {
      ...
      const options: MCPOptions = {
        type: 'websocket',
        url: 'wss://example.com/{{TENANT_ID}}/api/{{API_VERSION}}?user={{ALADIN_USER_ID}}&key=${TEST_API_KEY}',
      };
    ...
    it('should prioritize customUserVars over user fields and system env vars if placeholders are the same (though not recommended)', () => {
      ...
      process.env.ALADIN_USER_EMAIL = 'system-email-should-be-overridden';
      const user = createTestUser({ email: 'user-email-should-be-overridden' });
      const customUserVars = {
        ALADIN_USER_EMAIL: 'custom-email-wins',
      };
      const options: MCPOptions = {
        type: 'sse',
        url: 'https://example.com/api',
        headers: {
          'Test-Email': '{{ALADIN_USER_EMAIL}}', // Placeholder that could match custom, user, or system
        },
      };
      ...
      // Clean up env var
      delete process.env.ALADIN_USER_EMAIL;
    });
    ...
    it('should handle placeholders with no matching customUserVars (falling back to user/system vars)', () => {
      ...
      const options: MCPOptions = {
        ...
        url: 'https://example.com/api',
        headers: {
          'User-Email-Header': '{{ALADIN_USER_EMAIL}}', // Should use user.email
    ...
    it('should correctly process a mix of all variable types', () => {
      ...
      const obj = {
        type: 'streamable-http' as const,
        url: 'https://{{CUSTOM_ENDPOINT_ID}}.example.com/users/{{ALADIN_USER_USERNAME}}',
        headers: {
        ...
        env: {
          ...
          USER_HOME_DIR: '/home/{{ALADIN_USER_USERNAME}}',
    ...
    it('should process GitHub MCP server configuration with PAT_TOKEN placeholder', () => {
      ...
      // Simulate the GitHub MCP server configuration from aladin.yaml
      const options: MCPOptions = {
        type: 'streamable-http',
