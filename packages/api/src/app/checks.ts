import { logger, webSearchKeys } from '@aladin/data-schemas';
import { Constants, extractVariableName } from 'aladin-data-provider';
...
const deprecatedVariables = [
  {
    key: 'CHECK_BALANCE',
    description:
      'Please use the `balance` field in the `aladin.yaml` config file instead.\nMore info: https://aladin.ai/docs/configuration/aladin_yaml/object_structure/balance#overview',
  },
  {
    key: 'START_BALANCE',
    description:
...
function checkPasswordReset() {
  ...
  if (!emailEnabled && passwordResetAllowed) {
    logger.warn(
      ...
      https://www.aladin.ai/docs/configuration/authentication/email
...
export function checkVariables() {
  ...
  if (hasDefaultSecrets) {
    ...
    https://www.aladin.ai/toolkit/creds_generator
    ...
    if (typeof value === 'string') {
      ...
      if (varName) {
        ...
        logger.warn(
          ...
          This is incorrect! You should use environment variable references in your aladin.yaml file, such as:
          ${key}: "\${YOUR_ENV_VAR_NAME}"
          
          Then set the actual API key in your .env file or environment variables.
          
          More info: https://www.aladin.ai/docs/configuration/aladin_yaml/web_search`,
        );
      }
    }
  });
