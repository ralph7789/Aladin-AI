 ...
 * Priority:
 * 1. ALADIN_LOG_DIR environment variable
 * 2. If running within Aladin monorepo (when cwd ends with /api), use api/logs
...
export const getLogDirectory = (): string => {
  if (process.env.ALADIN_LOG_DIR) {
    return process.env.ALADIN_LOG_DIR;
  }
  ...
  // For Aladin project structure, use api/logs
  // For external consumers, they should set ALADIN_LOG_DIR
  if (cwd.includes('Aladin')) {
    return apiLogsPath;
  }
