# Aladin Helm Chart
This Librechat Helm Chart provides an easy, light weight template to deploy Aladin on Kubernetes

## Variables
In this Chart, Aladin will only work with environment Variables. You can Specify Vars and Secret using an existing Secret (This can be generated by [creating an Env File and converting it to a Kubernetes Secret](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#-em-secret-em-) `--from-env-file`)  
...
kind: Secret
metadata:
  name: aladin-credentials-env
  namespace: <aladin-chart-namespace>
type: Opaque
stringData:
...
Dependant of the Model you want to use, [create Credentials in your provider](https://docs.aladin.ai/install/configuration/ai_setup.html) and add them to the Secret:
```yaml
apiVersion: v1
kind: Secret
. . . .
