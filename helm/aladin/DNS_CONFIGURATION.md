# DNS Configuration for Aladin Helm Chart

This feature allows you to configure custom DNS settings for Aladin pods, enabling traffic redirection to proxy servers or custom endpoints.
...
Deploy:
```bash
helm upgrade --install aladin ./helm/aladin -f values.yaml
```
...
```bash
helm install aladin ./helm/aladin \
  --set dnsPolicy="None" \
...
✅ **Integration Test**
- Works with existing Aladin configuration
...
- Kubernetes 1.19+
- Compatible with all Aladin deployment modes
