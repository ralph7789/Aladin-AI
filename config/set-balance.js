...
const { getBalanceConfig } = require('@aladin/api');
const { User, Balance } = require('@aladin/data-schemas').createModels(mongoose);
...
(async () => {
  ...
  if (!balanceConfig?.enabled) {
    console.red(
      'Error: Balance is not enabled. Use aladin.yaml to enable it',
    );
    silentExit(1);
  }
