 ...
 * with subdirectory deployments (e.g., /aladin)
 */
...
describe('useAuthRedirect', () => {
  ...
  it('should respect router basename when redirecting (subdirectory deployment)', async () => {
    ...
    // Test with basename="/aladin" (simulates subdirectory deployment)
    const router = createTestRouter('/aladin');
    const { getByTestId } = render(<RouterProvider router={router} />);

    // Full pathname includes basename
    expect(router.state.location.pathname).toBe('/aladin/');

    // Wait for the redirect - router handles basename internally
    await waitFor(
      () => {
        // Router state pathname includes the full path with basename
        expect(router.state.location.pathname).toBe('/aladin/login');
        expect(getByTestId('login-page')).toBeInTheDocument();
      },
      { timeout: 1000 },
    );
