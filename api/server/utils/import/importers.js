...
const { EModelEndpoint, Constants, openAISettings, CacheKeys } = require('aladin-data-provider');
...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing Aladin conversation');
    return importAladinConvo;
  }
...
/**
 * Imports a Aladin conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importAladinConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
) {
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid Aladin file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from Aladin file`, error);
  }
}
