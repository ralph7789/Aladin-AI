...
const { EModelEndpoint, Constants, openAISettings } = require('aladin-data-provider');
...
describe('importAladinConvo', () => {
  const jsonDataNonRecursiveBranches = JSON.parse(
    fs.readFileSync(path.join(__dirname, '__data__', 'aladin-opts-nonr-branches.json'), 'utf8'),
  );
  ...
  it('should import conversation correctly', async () => {
    ...
    const jsonData = JSON.parse(
      fs.readFileSync(path.join(__dirname, '__data__', 'aladin-export.json'), 'utf8'),
    );
    const requestUserId = 'user-123';
    const importBatchBuilder = new ImportBatchBuilder(requestUserId);
  ...
  it('should import linear, non-recursive thread correctly with correct endpoint', async () => {
    ...
    const jsonData = JSON.parse(
      fs.readFileSync(path.join(__dirname, '__data__', 'aladin-linear.json'), 'utf8'),
    );
    const requestUserId = 'user-123';
    const importBatchBuilder = new ImportBatchBuilder(requestUserId);
  ...
  it('should maintain correct message hierarchy (tree parent/children relationship)', async () => {
    const jsonData = JSON.parse(
      fs.readFileSync(path.join(__dirname, '__data__', 'aladin-tree.json'), 'utf8'),
    );
    const requestUserId = 'user-123';
    const importBatchBuilder = new ImportBatchBuilder(requestUserId);
    jest.spyOn(importBatchBuilder, 'saveMessage');
