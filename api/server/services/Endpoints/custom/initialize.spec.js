...
jest.mock('@aladin/api', () => ({
  ...
  getCustomEndpointConfig: jest.fn().mockReturnValue({
    ...
    headers: { 'x-user': '{{ALADIN_USER_ID}}', 'x-email': '{{ALADIN_USER_EMAIL}}' },
...
describe('custom/initializeClient', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    const { getCustomEndpointConfig, resolveHeaders, getOpenAIConfig } = require('@aladin/api');
  ...
  it('throws if endpoint config is missing', async () => {
    const { getCustomEndpointConfig } = require('@aladin/api');
