...
jest.mock('aladin-data-provider', () => {
  const actual = jest.requireActual('aladin-data-provider');
  return {
    ...actual,
...
const { loadYaml } = require('@aladin/api');
...
describe('loadCustomConfig', () => {
  ...
  describe('parseCustomParams', () => {
    ...
    it('allows paramDefinitions with custom/unsupported keys if they are valid', async () => {
      const validCustomParams = {
        defaultParamsEndpoint: 'google',
        paramDefinitions: [
      ...
      const result = await loadCustomParams(validCustomParams);
