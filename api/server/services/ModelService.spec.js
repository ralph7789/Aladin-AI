...
const { logAxiosError, resolveHeaders } = require('@aladin/api');
const { EModelEndpoint, defaultModels } = require('aladin-data-provider');
...
jest.mock('@aladin/api', () => {
  const originalUtils = jest.requireActual('@aladin/api');
  return {
    ...originalUtils,
...
jest.mock('@aladin/data-schemas', () => ({
  ...jest.requireActual('@aladin/data-schemas'),
  logger: {
    error: jest.fn(),
  },
}));
...
describe('fetchModels with createTokenConfig true', () => {
  ...
  beforeEach(() => {
    ...
    const _utils = require('@aladin/api');
  ...
  it('creates and stores token configuration if createTokenConfig is true', async () => {
    ...
    const { processModelData } = require('@aladin/api');
...
describe('getOpenAIModels with mocked config', () => {
  it('uses alternative behavior when userProvidedOpenAI is true', async () => {
    ...
    jest.mock('aladin-data-provider', () => {
      const original = jest.requireActual('aladin-data-provider');
      return {
        ...original,
        defaultModels: {
