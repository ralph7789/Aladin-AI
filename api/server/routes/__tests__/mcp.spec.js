...
describe('MCP Routes', () => {
  ...
  describe('GET /:serverName/oauth/initiate', () => {
    const { MCPOAuthHandler, mcpServersRegistry } = require('@aladin/api');
  ...
  describe('GET /:serverName/oauth/callback', () => {
    const { MCPOAuthHandler, MCPTokenStorage } = require('@aladin/api');
    const { getLogStores } = require('~/cache');
    ...
    it('should handle OAuth callback successfully', async () => {
      const { mcpServersRegistry } = require('@aladin/api');
      const mockFlowManager = {
  ...
  describe('POST /oauth/cancel/:serverName', () => {
    const { MCPOAuthHandler } = require('@aladin/api');
    const { getLogStores } = require('~/cache');
  ...
  describe('POST /:serverName/reinitialize', () => {
    ...
    it('should handle server with custom user variables', async () => {
      ...
      require('~/config').getFlowStateManager.mockReturnValue({});
      require('~/cache').getLogStores.mockReturnValue({});
      require('@aladin/api').getUserMCPAuthMap.mockResolvedValue({
        'mcp:test-server': {
      ...
      expect(require('@aladin/api').getUserMCPAuthMap).toHaveBeenCalledWith({
        userId: 'test-user-id',
        servers: ['test-server'],
  ...
  describe('GET /:serverName/oauth/callback - Edge Cases', () => {
    it('should handle OAuth callback without toolFlowId (falsy toolFlowId)', async () => {
      const { MCPOAuthHandler, MCPTokenStorage, mcpServersRegistry } = require('@aladin/api');
      const mockTokens = {
